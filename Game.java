...
public class Game {
    ...
    public static void GO(String direction) {
        ...
        direction = direction.trim().toUpperCase(); // Trim whitespace and convert to uppercase
    ...
    public static void respond(String in) {
        in = in.trim().toUpperCase(); // Trim whitespace from input

        if (in.startsWith("GO ")) { // Check if the input starts with "GO "
            GO(in);
        } else if (in.startsWith("LOOK ")) {
            LOOK(in.substring(5).trim()); // Trim the item name
        } else if (in.startsWith("TAKE ")) {
            TAKE(in.substring(5).trim()); // Trim the item name
        } else if (in.startsWith("DROP ")) {
            DROP(in.substring(5).trim()); // Trim the item name
        } else if (in.startsWith("USE ")) {
            USE(in.substring(4).trim()); // Trim the item name
        } else if (in.startsWith("TALK ")) {
            TALK(in.substring(5).trim()); // Trim the NPC name
        } else if (in.equals("INVENTORY")) { // Use .equals() for exact command
            INVENTORY();
        } else if (in.equals("HELP")) { // Use .equals() for exact command
            HELP();
        } else {
    ...
    public static void DROP(String item) {
        item = item.trim().toUpperCase(); // Trim whitespace
    ...
    public static void LOOK(String item) {
        ...
        if (map.getRow() == 0) {
            ...
            if (map.getCol() == 0) {
                object.add("Shallow River: A shallow river that looks like it goes on forever. It winds and bends several times. You wouldn't want to get lost in the river.");
        ...
        item = item.trim().toUpperCase(); // Trim whitespace
        if (!object.isEmpty()) {
            for (String objStr : object) {
                String itemName = objStr.substring(0, objStr.indexOf(":")).trim().toUpperCase();
                String description = objStr.substring(objStr.indexOf(':') + 2).trim();
    ...
    public static void TAKE(String item) {
        ...
        if (player.hasItem(item)) {
            ...
            for (String interaction : interactions) {
                if (item.equals(interaction.trim().toUpperCase())) {
                    player.addItem(interaction.trim().toUpperCase());
                    System.out.println("You have taken " + interaction + ".");
                    return;
                }
            }
    ...
    public static void USE(String item) {
        ...
        if (player.hasItem(item) && item.equals("COMPASS")) {
            ...
            if (map.getRow() == 2 && map.getCol() == 0) {
                ...
                System.out.println("With the rusty sword, you bravely defeat the dragon and claim the Golden Amulet!");
            } else {
                System.out.println("You can't use this item here.");
            }
            ...
            System.out.println("You don't have that item or can't use it right now.");
        }
